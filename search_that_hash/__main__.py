	...
	if not sth_results == False:
		for result in sth_results.values():
			if not config['greppable']:
				if not config['api']:
					printing.Prettifier.sth_print(result['Hash'],result['Plaintext'],result['Type'],result['Verified'])
				else:
		...
		for hash in hash_processes:
			...
			try:
				...
				if "STH_API" not in list(base_results[0].keys()) and base_results[0] != {}:
					sth.push(config, list(hash.keys())[0], list(base_results[0].values())[0], types)
