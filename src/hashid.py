...
prototypes = [
    ...
    Prototype(
        ...
        modes=[
            ...
            HashInfo(
                name="SHA3-256", hashcat=5000, john="raw-keccak-256", extended=False
            ),
...
def main():
    usage = "{0} [-h] [-e] [-m] [-j] [-o FILE] [--version] INPUT".format(
    ...
    group.add_argument(
        "-o", "--outfile", metavar="FILE", type=str, help="write output to file"
    )
    group.add_argument(
        "-h", "--help", action="help", help="show this help message and exit"
    )
    group.add_argument("--version", action="version", version=__banner__)
    ...
    if not args.strings or args.strings[0] == "-":
        while True:
            ...
            writeResult(
                hashID.identifyHash(line), outfile, args.mode, args.john, args.extended
            )
            sys.stdout.flush()
    else:
